...
def bureau_and_balance(num_rows = None, nan_as_category = True):
    ...
    gc.collect()
    bureau_agg.to_csv("./data/bureau_agg.csv",index=False)
    return bureau_agg
...
def previous_applications(num_rows = None, nan_as_category = True):
    ...
    gc.collect()
    prev_agg.to_csv("./data/prev_agg.csv",index=False)
    return prev_agg
...
def pos_cash(num_rows = None, nan_as_category = True):
    ...
    del pos
    gc.collect()
    pos_agg.to_csv("./data/pos_agg.csv",index=False)
    return pos_agg
...
def installments_payments(num_rows = None, nan_as_category = True):
    ...
    del ins
    gc.collect()
    ins_agg.to_csv("./data/ins_agg.csv",index=False)
    return ins_agg
...
def credit_card_balance(num_rows = None, nan_as_category = True):
    ...
    gc.collect()
    cc_agg.to_csv("./data/cc_agg.csv",index=False)
    return cc_agg
...
def lightgbm(df, debug= False):
    ...
    #tr = pd.read_feather("../data/corresponding_aggregate_train.ftr")
    #te = pd.read_feather("../data/corresponding_aggregate_test.ftr")
    ...
    print('Full AUC score %.6f' % roc_auc_score(valid_y, oof_preds))
...
def main(debug = False):
    ...
    with timer("Run LightGBM with kfold"):
        feat_importance = lightgbm(df,debug= debug)
    with timer("OUTPUT IMPORTANCE"):
        feat_importance.to_csv("IMPOR.csv")
